...
let bobbingTime = 0;
let baseY = null; // Store the base Y position
let wasMoving = false; // Track previous movement state
const bobbingSpeed = 7; // How fast the bobbing occurs
const bobbingAmount = 0.08; // How much the camera bobs up and down (subtle)
...
export const updateMovement = (delta, controls, camera, walls) => {
  ...
  const isMoving = keysPressed.ArrowRight || keysPressed.d || 
                   keysPressed.ArrowLeft || keysPressed.a || 
                   keysPressed.ArrowUp || keysPressed.w || 
                   keysPressed.ArrowDown || keysPressed.s;

  // Store the base Y position when starting to move
  if (isMoving && !wasMoving) {
    baseY = camera.position.y;
  }

  // Reset to base Y position when stopping
  if (!isMoving && wasMoving) {
    if (baseY !== null) {
  ...
  // Apply camera bobbing effect when moving
  if (isMoving && baseY !== null) {
    bobbingTime += delta * bobbingSpeed;
    const bobbingOffset = Math.sin(bobbingTime) * bobbingAmount;
    camera.position.y = baseY + bobbingOffset; // Set position relative to base, not adding
  }
